...
Data0_rf$res.1d <- c(residualsCV[1], residualsCV[1:(length(residualsCV)-1)])
Data0_rf$res.7d <- c(residualsCV[1:7], residualsCV[1:(length(residualsCV)-7)])

Data1_rf <- data.frame(Data1, terms1)
colnames(Data1_rf)
residuals <- Data1_rf$Load - gam9.forecast
Data1_rf$residuals <- residuals
Data1_rf$res.1d <- c(residuals[1], residuals[1:(length(residuals)-1)])
Data1_rf$res.7d <- c(residuals[1:7], residuals[1:(length(residuals)-7)])

cov <- "Time + toy + Temp + Load.1 + Load.7 + Temp_s99 + WeekDays + BH + Temp_s95_max + Temp_s99_max + Summer_break  + Christmas_break + 
Temp_s95_min +Temp_s99_min + DLS + GovernmentResponseIndex + res.1d + res.7d +"
...
Data_a$res.1d <- c(Data_a$residuals[1], Data_a$residuals [1:(length(Data_a$residuals )-1)])
Data_a$res.7d <- c(Data_a$residuals[1:7], Data_a$residuals [1:(length(Data_a$residuals )-7)])

Data_b$res.1d <- c(Data_b$residuals[1], Data_b$residuals [1:(length(Data_b$residuals )-1)])
Data_b$res.7d <- c(Data_b$residuals[1:7], Data_b$residuals [1:(length(Data_b$residuals )-7)])
...
equation_res_newdata <- paste0("residuals~  Time + toy + Temp + Load.1 + Load.7 + Temp_s99 + WeekDays + BH + Temp_s95_max + Temp_s99_max + Summer_break  + Christmas_break + 
  Temp_s95_min +Temp_s99_min + DLS + GovernmentResponseIndex + res.1d + res.7d + ", '+', paste0(newdata, collapse  ='+'))
