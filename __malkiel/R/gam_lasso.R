...
Data2<-read_delim("Data/test.csv", delim=",")
submit1 <- read_delim( file="Data/sample_submission.csv", delim=",")
submit1$Load <- predict(gfit$gam, newdata=Data2)
write.table(submit1, file="Data/submission_gamlasso.csv", quote=F, sep=",", dec='.',row.names = F)
